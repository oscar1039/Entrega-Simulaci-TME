...
while True:
    ...
    if (random()<= ProbTermAnders):
        sigma = np.sqrt(k*T/mass)
        Randomatom = np.random.randint(0,Natoms)
        pmodantic = p[Randomatom].mag
        punitari = p[Randomatom]/pmodantic

        px = np.random.normal(0, sigma)*mass
        py = np.random.normal(0, sigma)*mass
        pz = np.random.normal(0, sigma)*mass

        pmodnou = np.sqrt(px**2 + py**2 + pz**2)
        p[Randomatom] = punitari*pmodnou
