...
deltacomp = 100
vmaxcomp = 3000
nbinscomp = int(2*vmaxcomp/deltacomp)

def barcomp(v):
    return int((v + vmaxcomp)/deltacomp)

gx = graph(width=win, height=0.3*win, xmin=-vmaxcomp, xmax=vmaxcomp,
           align='left', xtitle='vx (m/s)', ytitle='Number of atoms')
vxbar = gvbars(color=color.red, delta=deltacomp)

gy = graph(width=win, height=0.3*win, xmin=-vmaxcomp, xmax=vmaxcomp,
           align='left', xtitle='vy (m/s)', ytitle='Number of atoms')
vybar = gvbars(color=color.green, delta=deltacomp)

gz = graph(width=win, height=0.3*win, xmin=-vmaxcomp, xmax=vmaxcomp,
           align='left', xtitle='vz (m/s)', ytitle='Number of atoms')
vzbar = gvbars(color=color.blue, delta=deltacomp)


deltapos = 0.05
posmax = L/2
nbinspos = int(2*posmax/deltapos)

def barpos(x):
    return int((x + posmax)/deltapos)

gxpos = graph(width=win, height=0.3*win, xmin=-posmax, xmax=posmax,
              align='left', xtitle='x position (m)', ytitle='Number of atoms')
xposbar = gvbars(color=color.red, delta=deltapos)

gypos = graph(width=win, height=0.3*win, xmin=-posmax, xmax=posmax,
              align='left', xtitle='y position (m)', ytitle='Number of atoms')
yposbar = gvbars(color=color.green, delta=deltapos)

gzpos = graph(width=win, height=0.3*win, xmin=-posmax, xmax=posmax,
              align='left', xtitle='z position (m)', ytitle='Number of atoms')
zposbar = gvbars(color=color.blue, delta=deltapos)
...
while True:
    ...
    if nhisto % 10 == 0:
        ...
        for j in range(nbinscomp):
            vcenter = -vmaxcomp + deltacomp*(j + 0.5)

            datax.append([vcenter, histox[j]])
            datay.append([vcenter, histoy[j]])
            dataz.append([vcenter, histoz[j]])

        vxbar.data = datax
        vybar.data = datay
        vzbar.data = dataz
        histoxpos = [0]*nbinspos
        histoypos = [0]*nbinspos
        histozpos = [0]*nbinspos
        ...
        for j in range(nbinspos):
            xcenter = -posmax + deltapos*(j + 0.5)

            dataxpos.append([xcenter, histoxpos[j]])
            dataypos.append([xcenter, histoypos[j]])
            datazpos.append([xcenter, histozpos[j]])

        xposbar.data = dataxpos
        yposbar.data = dataypos
        zposbar.data = datazpos
